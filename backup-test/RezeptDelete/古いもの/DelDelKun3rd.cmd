...
rem 3分割してみる(78,130,597－39,000,000－39,000,000－130,597＝0)
rem【3回目】39,000,000件削除(端数も含めて全部処理)※本当は、130,597件なのだが、消せていないかもしれないから
rem      開始時対象レコードカウント：しない(インデックスないから)
rem      終了時対象レコードカウント：する(インデックスあるから)
rem      インデックス削除処理：しない(途中だから)
rem      インデックス作成処理：する(おわったから)
rem      統計情報取得処理：する(おわったからEXEC sp_updatestats)
